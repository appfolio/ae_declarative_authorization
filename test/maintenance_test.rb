...
class MaintenanceTest < Test::Unit::TestCase
  ...
  def test_without_access_control
    ...
    without_access_control # coverage
  end

  def test_with_user
    original_user = Authorization.current_user
    user = MockUser.new(:test_role)
    Authorization::Maintenance.with_user(user) do
      assert_equal user, Authorization.current_user
    end
    assert_equal original_user, Authorization.current_user

    with_user(user) # coverage
  end
end
